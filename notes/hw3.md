...
# 💻 陣列 (Array) 與 鏈結串列 (Linked List) 對比分析

這個表格比較了兩種基礎資料結構——陣列和鏈結串列——在各個方面的特性，以及它們常見操作的時間複雜度 (Time Complexity)。

## 一、結構特性比較 (Structural Comparison)

| 方面 (Aspect) | 陣列 (Array) | 鏈結串列 (Linked List) |
| :--- | :--- | :--- |
| **儲存方式** (Storage) | **連續的記憶體空間 (Contiguous Memory)** | **非連續的記憶體空間 (Non-Contiguous)** |
| **彈性/大小** (Flexibility) | **大小固定**；需要重新分配記憶體 (Reallocation) 才能擴展。 | **大小動態變化**；易於擴展或縮小。 |
| **記憶體開銷** (Overhead) | **開銷最小** (僅儲存資料本身)。 | **額外開銷** (每個節點需儲存指向下一個節點的指標)。 |
| **走訪方式** (Traversal) | **簡單的索引迭代 (Index Iteration)**。 | **指標追蹤 (Pointer Chasing)**；可能因快取 (Cache) 效率低而較慢。 |
| **變數舉例** (Extra Variables) | `i`, `j`, `min_idx`, `MAX_SIZE` (索引、最大容量) | `head`, `current`, `next`, `min_node` (頭節點、指標) |
| **優勢場景** (Best For) | **大小固定**且需要頻繁進行**讀取/存取**的場景。 | **動態資料**且需要頻繁進行**插入/刪除**的場景。 |

## 二、操作時間複雜度 (Big O Time Complexity)

時間複雜度是衡量演算法效率的標準。以下比較了在 $N$ 個元素的資料結構中，執行各項操作所需的**最差情況**時間。

| 操作 (Operation) | 陣列 (Array) | 鏈結串列 (Linked List) | 備註 (Notes) |
| :--- | :--- | :--- | :--- |
| **存取元素** (Access by Index) | **$O(1)$** (常數時間) | **$O(N)$** (線性時間) | 陣列的優勢：直接計算記憶體地址。 |
| **在開頭插入/刪除** (Insert/Delete at Head) | **$O(N)$** (需移動所有元素) | **$O(1)$** (常數時間) | 鏈結串列的巨大優勢：只需更改 `head` 指標。 |
| **在末尾插入** (Insert at Tail) | **$O(1)$** (若有 `size` 變數) | **$O(N)$** (對於單向) 或 **$O(1)$** (對於雙向且有 `tail` 指標) | 需考量是否為單向或雙向。 |
| **在中間插入/刪除** (Insert/Delete Mid-way) | **$O(N)$** (平均需移動一半元素) | **$O(N)$** (需先走訪到該位置) | 兩者皆需先找到位置，但鏈結串列實際操作速度可能因指標追蹤而稍慢。 |
| **搜尋元素** (Search for a Value) | $O(N)$ | $O(N)$ | 兩者都需要遍歷所有元素。 |
